...
#dir  <- paste0("/Users/asanc31/Library/CloudStorage/",
#          "OneDrive-EmoryUniversity/Research assistant/expert_data/")
dir <- "D:/OneDrive - Emory/QTM 499R/Article_Reference_OpenAlex/"
...
for (j in seq(1,num_total_article,batch_size)){
  ...
  filter_batch        <-   oa_fetch(entity = "works",
                                    doi = doi_batch,
                                    options = list(select = c("doi", "id", "type"))) %>%
  ...
  if(length(missing_DOI) >=1 ){
    for(k in 1:length(missing_DOI)){
      ...
      fixed_records  <- oa_fetch(entity = "works",
                                 options = list(select = c("doi", "id", "type",
                                                           "publication_date",
                                                           "author",
                                                           "display_name",
                                                           "referenced_works")),
                                 title.search = missing_titles[k]) %>%
